...
test('test', async ({ page }) => {
    ...
    await page.goto('https://dev8.opdev.vn/bank-emulator/emulator-mpgs/?url=https%3A%2F%2Fdev8.opdev.vn%2Fpaygate%2Fapi%2Fv1%2Fauthorizations%2FAUT-3KKg3jnITVaf53xdLLWd2A%3Finvoice_id%3DINV-3MjpZuSGSUOszKqNDuGHeQ%26payment_id%3DPAY-a07jWs7dTD2H_W7hXJdffQ%26brand_id%3Dmastercard%26acq%3DMPGS%26merchant%3DTESTOPTEST%26merchTxnRef%3DPAY-a07jWs7dTD2H_W7hXJdffQ%26version%3D3DS2&order.id=PAY-a07jWs7dTD2H_W7hXJdffQ&response.gatewayRecommendation=PROCEED&result=SUCCESS');
    await page.getByRole('button', { name: 'Submit' }).click();
    await expect(page.getByRole('heading', { name: 'Giao dịch thành công' })).toBeVisible();
    await expect(page.getByRole('cell', { name: 'Giao dịch thành công' })).toBeVisible();
});