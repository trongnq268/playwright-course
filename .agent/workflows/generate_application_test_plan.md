...
4. **BẮT BUỘC Chờ user xác nhận** scope trước khi sang Bước 3
...
|---|---|---|---|---|---|---|---|
| New | User Management | TC_LOGIN_001 - Successful login with valid email | User has a registered account; system is running | @regression | Enter email: test@example.com | System displays "Login successful" | Manual |

   - **Automation Candidates** — đánh dấu scenarios nào nên tự động hóa và lý do
2. Nếu user chọn **Mode PLAN** → **KẾT THÚC** tại đây
3. **BẮT BUỘC Chờ user xác nhận** scope trước khi sang Bước 5

### Bước 5: Sinh Manual Test Cases (Mode FULL)

> Chỉ thực hiện khi ở **Mode FULL**

1. Chuyển test scenarios (Bước 3) thành **manual test cases đầy đủ** sử dụng format:

| CRUD | Test Suite | Test Case Name | Preconditions | Tag Name | Action | Expected Result | Execution Type |
|---|---|---|---|---|---|---|---|
| New | Module Name | TC_ID - Title | ... | ... | ... | ... | ... |

> **Note:** Each TC spans multiple rows, one per step. The columns `CRUD`, `Test Suite`, `Test Case Name`, `Preconditions`, `Tag Name`, and `Execution Type` repeat the same value for each step row belonging to the same TC.
